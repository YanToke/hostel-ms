...
export async function PUT(request, { params }) {
  try {
    ...
    if (
      ...
      img &&
      img.name !== '' &&
      `/building-images/${img.name}` === building.img
    ) {
    ...
    const isSuccess = await updateBuilding(id, name, '/building-images/' + img)
    if (isSuccess) {
      return NextResponse.json(
      ...
      return NextResponse.json(
        { message: 'Error in updating building' },
        { status: 500 }
      )
    }
  } catch (error) {
