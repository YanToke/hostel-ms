...
export async function PUT(request, { params }) {
    try {
        ...
        if (!floor) {
            return NextResponse.json({ message: "Floor doesn't exist" }, { status: 400 });
        }
        const formData = await request.formData();
        let { buildingId,name, img} = getDataFromForm(formData,"buildingId", "name", "img")
        console.log("Image is ", img);
        if (img && img.name !== '' && `/floor-images/${img.name}` === '/floor-images/floor.jpg') {
            console.log("If statement")
            console.log("Env image is",process.env.DEFAULT_FLOOR_IMAGE)

            img = process.env.DEFAULT_FLOOR_IMAGE;
        }
        else if (img && img.name !== '' && `/floor-images/${img.name}` === floor.img) {
            console.log("If else 1")
            img = img;
        }
        else if (img && img.name !== '' && `/floor-images/${img.name}` !== '/floor-images/floor.jpg') {
        ...
        const isSuccess = await updateFloor(id,buildingId,name, '/floor-images/' + img);
        if (isSuccess) {
            return NextResponse.json({ message: "Successfully updated" }, { status: 200 });
        }
        else {
            return NextResponse.json({ message: "Error in updating floor" }, { status: 500 })
        }
